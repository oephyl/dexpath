...
import { calculateRugcheckTrustScore, getTrustColor } from "@/features/rugcheck/utils/trustScore"
...
export default function TokenDetailClient({ address }: { address: string }) {
  ...
  useEffect(() => {
    ...
    const fetchRugReport = async () => {
      try {
        ...
        if (!res.ok) {
          if (res.status === 429) {
            // Rate limited: respect Retry-After header if present, otherwise backoff
            const ra = res.headers.get("retry-after")
            const retryMs = ra && !isNaN(Number(ra)) ? Number(ra) * 1000 : Math.min((rugBackoffRef.current || 1000) * 2, 60000)
            if (rugIntervalRef.current) {
              window.clearInterval(rugIntervalRef.current)
              rugIntervalRef.current = null
            }
            rugBackoffRef.current = retryMs
            rugIntervalRef.current = window.setInterval(fetchRugReport, retryMs)
            if (!aborted) setRugError(`Rate limited — retrying in ${Math.round(retryMs / 1000)}s`)
            console.warn(`[v0] Rugcheck rate limited, retrying in ${retryMs}ms`)
            return
          }
  ...
  const rugcheckSummary = useMemo(() => {
    ...
    const summaryText = `${risks.length} risk(s) detected — ${danger} danger level`
    return { summaryText, risks }
  }, [rugReport])

  const rugcheckChecklist = useMemo(() => {
    if (!rugReport) return null
    return {
      markets: rugReport?.markets ?? [],
      audits: rugReport?.audits ?? [],
      checklist: rugReport?.checklist ?? [],
      info: rugReport?.info ?? null,
    }
  }, [rugReport])

  const bondingState = useMemo(() => {
    const bc: any =
      (token as any)?.bondingCurve ??
      // mobula may expose bonding info in several shapes
      mobulaDetails?.data?.data?.bonding ??
      mobulaDetails?.data?.bonding ??
      // some responses include bondingPercentage as top-level numeric percent
      (mobulaDetails?.data?.bondingPercentage != null ? { percent: mobulaDetails.data.bondingPercentage } : null) ??
      (mobulaDetails?.data?.data?.bondingPercentage != null ? { percent: mobulaDetails.data.data.bondingPercentage } : null) ??
      (mobulaDetails?.bondingPercentage != null ? { percent: mobulaDetails.bondingPercentage } : null) ??
      ((token as any)?.bondingPercentage != null ? { percent: (token as any).bondingPercentage } : null) ??
      mobulaDetails?.bonding ??
      null

    if (!bc) return { percent: null, bonded: false }
    ...
    if (typeof bc === 'number') raw = bc
    else if (bc && (bc.bondingPercentage != null || bc.bonding_percentage != null || bc.percent != null)) {
      raw = toNumber(bc.bondingPercentage) ?? toNumber(bc.bonding_percentage) ?? toNumber(bc.percent)
    } else {
      raw = toNumber(bc.progress) ?? toNumber(bc.percent) ?? toNumber(bc.progressPct) ?? toNumber(bc.progress_pct) ?? toNumber(bc.progressPercent)
    }

    const bondedFlag = !!(
      bc?.bonded ||
      mobulaDetails?.data?.bonded ||
      mobulaDetails?.data?.data?.bonded ||
      mobulaDetails?.bonded ||
      (token as any)?.bonded
    )

    if (raw == null || Number.isNaN(Number(raw))) {
      if (bondedFlag) return { percent: 100, bonded: true }
      return { percent: null, bonded: false }
    }

    let num = Number(raw)
    if (num <= 1 && num >= 0) num = Math.round(num * 100)
    let percent = Math.max(0, Math.min(100, Math.round(num)))
    if (bondedFlag) percent = 100
    const bonded = percent >= 100 || bondedFlag
    return { percent, bonded }
  }, [token, mobulaDetails])
  ...
  // Safe token description derived from API data or token object
  const tokenDescription = useMemo(() => {
    const data = mobulaDetails?.data?.data ?? mobulaDetails?.data ?? mobulaDetails
    const raw = (token as any)?.description ?? data?.description ?? data?.overview ?? ""
  ...
  const descriptionPreview = useMemo(() => {
    ...
    const max = 240
    return tokenDescription.length > max ? `${tokenDescription.slice(0, max)}...` : tokenDescription
  }, [tokenDescription])

  // Derived/UI-safe values used by JSX
  const dexscreenerHeader = useMemo(() => {
    const data = mobulaDetails?.data ?? mobulaDetails?.data?.data ?? mobulaDetails ?? {}
    const listed = Boolean(data?.dexscreenerListed ?? mobulaDetails?.dexscreenerListed ?? false)
    const dsHeader = mobulaDetails?.data?.dexscreenerHeader ?? mobulaDetails?.dexscreenerHeader ?? data?.dexscreenerHeader ?? data?.dexscreener?.headerImage ?? data?.dexscreener?.image ?? null
    const header =
      dsHeader ??
      data?.headerImage ??
      data?.header ??
      ...
      (token as any)?.headerImage ??
      ""
    if (listed) return dsHeader ?? header ?? (token as any)?.logo ?? ""
    return header
  }, [mobulaDetails, token])

  const socialLinks = useMemo(() => {
    const raw =
      mobulaDetails?.data?.socials ??
      mobulaDetails?.data?.links ??
      mobulaDetails?.data?.data?.socials ??
      (token as any)?.socials ??
      []

    const normalize = (key: string, href: any) => {
      if (!href) return null
      const url = typeof href === "string" ? href : href.href || href.url || href.link
      if (!url || typeof url !== "string") return null
      const k = (key || url).toString()
      const label = (key && key.toString()) || url
      let Icon: ElementType = Link2
      const kl = k.toLowerCase()
      if (kl.includes("twitter") || kl.includes("x.com")) Icon = Twitter
      else if (kl.includes("github")) Icon = Github
      else if (kl.includes("web") || kl.includes("site") || kl.includes("website") || kl.includes("instagram")) Icon = Globe
      return { key: k, href: url, label, Icon }
    }
    ...
    if (Array.isArray(raw)) {
      return raw
        .map((s: any) => {
          if (!s) return null
          if (typeof s === "string") return normalize(s, s)
          const href = s.href || s.url || s.link || s
          const key = s.key || s.type || s.name || href
          return normalize(key, href)
        })
        .filter(Boolean)
    }
  ...
  const dexscreenerBoostedAmount = useMemo(() => {
    return mobulaDetails?.data?.boostedAmount ?? (token?.boostCount ?? 0)
  }, [mobulaDetails, token])

  const analyzerSummary = useMemo(() => {
    try {
      const inputs = extractMobulaTokenDetailsInputs(mobulaDetails ?? token ?? {})
      return analyzeToken(inputs).summary ?? []
    } catch (e) {
      return []
    }
  }, [mobulaDetails, token])
  ...
  const dexPaid = useMemo(() => {
    return dexEvents.filter((e) => e.type === "DEXAD_PAID").length
  }, [dexEvents])

  const rugcheckScore = useMemo(() => {
    if (!rugReport) return null
    const score = Number(rugReport?.score ?? rugReport?.riskScore ?? rugReport?.scoreTotal ?? 0)
    const rugged = score < 50
    const scoreColor = rugged ? "bg-red-600" : "bg-green-600"
    return { score, rugged, scoreColor }
  }, [rugReport])

  const rugcheckInfo = useMemo(() => {
    return rugReport?.info ?? rugcheckChecklist?.info ?? null
  }, [rugReport, rugcheckChecklist])

  const trust = useMemo(() => calculateRugcheckTrustScore(rugReport ?? {}), [rugReport])
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-2 sm:px-4 py-4 sm:py-6 max-w-[1600px]"> 
        ...
        {/* Chart + Sidebar layout: 8/4 columns */}
        <div className="space-y-6 mb-6">
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <div className="flex space-x-2">
                      <Button size="sm" variant={sideTab === 'details' ? 'default' : 'ghost'} onClick={() => setSideTab('details')}>Token Detail</Button>
                      <Button size="sm" variant={sideTab === 'rugcheck' ? 'default' : 'ghost'} onClick={() => setSideTab('rugcheck')}>Rugcheck</Button>
                      <Button size="sm" variant={sideTab === 'timeline' ? 'default' : 'ghost'} onClick={() => setSideTab('timeline')}>Analyzer</Button>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                            ...
                            <div className="space-y-2">
                              ...
                              {analyzerSummary.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                  {analyzerSummary.slice(0, 6).map((line, idx) => (
                                    <div key={`summary-${idx}`} className="text-sm text-muted-foreground">{line}</div>
                                  ))}
                                </div>
                              )}
                            </div>
                      ...
                      <CardContent>
                        {rugReport ? (
                          <div>
                            ...
                            {trust && (
                              <div className="mb-6">
                                <div className="text-zinc-400 text-sm mb-2">Internal safety analysis based on Rugcheck data</div>
                                <div className="grid grid-cols-2 gap-4 items-center">
                                  <div className="flex items-center gap-3">
                                    <div className={`${getTrustColor(trust.score)} p-2 rounded-full inline-flex items-center justify-center`}>
                                      <ShieldCheck className="h-5 w-5" />
                                    </div>
                                    <div>
                                      <div className="text-sm font-semibold">Trust Score</div>
                                      <div className="text-lg font-bold">{trust.score}/100</div>
                                    </div>
                                  </div>

                                  <div className="flex items-center gap-3">
                                    <div className="p-2 rounded-full bg-zinc-800 inline-flex items-center justify-center">
                                      <InfoIcon className="h-5 w-5 text-zinc-200" />
                                    </div>
                                    <div>
                                      <div className="text-sm font-semibold">{trust.label}</div>
                                      <div className="text-sm text-zinc-400">
                                        {Object.entries(trust.breakdown).map(([k, v]) => `${k}: ${v}`).join(" • ")}
                                      </div>
                                    </div>
                                  </div>
                                </div>
                            ...
                            {rugError && <p className="text-yellow-500 text-sm">{rugError}</p>}

                            {/* Show source when available (restored) */}
                            {(rugReport?.source || rugReport?.meta?.source || rugReport?.info?.source || rugReport?.dataSource) && (
                              <div className="text-xs text-muted-foreground mb-2">
                                Source: {rugReport?.source ?? rugReport?.meta?.source ?? rugReport?.info?.source ?? rugReport?.dataSource}
                              </div>
                            )}
                              ...
                              <div className="space-y-4">
                                ...
                                {rugcheckChecklist && (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {rugcheckChecklist.checklist.map((item: any, idx: number) => (
                          ...
                          <p className="text-muted-foreground text-sm">Loading rugcheck…</p>
                        ) : rugError ? (
                          <p className="text-red-500 text-sm">{rugError}</p>
                        ) : null}
                      </CardContent>
                    </Card>
                  )}
